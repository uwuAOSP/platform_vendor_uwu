...
func (m *kernelModule) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if ctx.Config().ProductVariables().BoardUsesSoongKernel {
		...
		if m.dtboImage.Valid() && ctx.Config().KatiEnabled() {
...
func (m *kernelModule) buildModules(ctx android.ModuleContext, source, buildRoot, arch string, dependency android.Path, inputs android.Paths) android.Path {
	...
	for _, set := range moduleSets {
		...
		cmd.Text("&& missing=$(awk 'NR==FNR { seen[$0]=1; next } !($0 in seen)'").Text(filepath.Join(flat.String(), installFile)).Text(filepath.Join(flat.String(), loadFile)).Text("); test -z \"$missing\" || { echo \"modules in load list but not install list: $missing\" >&2; exit 1; }")
...
func (m *kernelModule) writeAllVendorModuleList(cmd *android.RuleBuilderCommand, dir, name, excluded string, excludeSystem bool) {
	...
	if excludeSystem {
		cmd.Text("&& awk 'NR==FNR { seen[$0]=1; next } !($0 in seen)'").Text(excluded).Text(all).Text("| sort -u >").Text(path)
