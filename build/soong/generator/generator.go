...
func init() {
	android.RegisterModuleType("uwu_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = uwuExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = uwuExpandVariables(ctx, dep_file)
	...
	cmd := uwuExpandVariables(ctx, String(g.properties.Cmd))
