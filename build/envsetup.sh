...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^uwu_") ; then
        UWU_BUILD=$(echo -n $1 | sed -e 's/^uwu_//g')
    else
        UWU_BUILD=
    fi
    export UWU_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/uwu/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the uwuAOSP model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/*-ota.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.uwu.device | grep -q "$UWU_BUILD"); then
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.uwu.device | grep -q "$UWU_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.uwu.device | grep -q "$UWU_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
